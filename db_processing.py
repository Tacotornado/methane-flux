...
def extract_chamber_ch4(con, result_dir):
    ...
    session_count = 0
    current_chamber = all_active_sessions.iloc[0]['ChamberID'] if len(all_active_sessions) > 0 else None
    ...
    for i in tqdm(range(_len), desc="Querying active sessions"):
        ...
        start_time_dt = datetime.fromisoformat(str(start_time)) + timedelta(seconds=PURGE_BUFFER)
